...
function findGenerateButton(input) {
  ...
  let b = buttons.find((x) => x.innerHTML.includes('arrow_forward'));
  if (b) return b;

  // 2) Tìm nút chứa chữ "Generate" (tiếng Anh)
  b = buttons.find((x) => {
  ...
  // 3) Tìm nút chứa chữ "Tạo" nhưng không có icon "add" (loại trừ nút Tạo dialog)
  b = buttons.find((x) => {
    const txt = (x.textContent || x.innerText || "").toLowerCase();
    ...
    if (msg.type === "SUBMIT_PROMPT") {
      ...
      (async () => {
        try {
          ...
          let input = await wakePromptBox();
          if (!input) {
            sendResponse({ ok: false, error: "Không tìm thấy ô prompt trên màn hình." });
            return;
          }
          ...
          // 4. Bấm nút Tạo / Gửi
          const sent = await submitPromptReliably(input);
          if (sent) {
            sendResponse({ ok: true });
          } else {
            sendResponse({ ok: false, error: "Đã gõ chữ nhưng không gửi được (nút gửi bị khóa hoặc Enter không nhạy)." });
          }
        } catch (e) {
          sendResponse({ ok: false, error: "Lỗi thực thi: " + String(e) });
        }
      })();
