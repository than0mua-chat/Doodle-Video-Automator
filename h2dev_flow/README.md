...
> **CÁCH HOẠT ĐỘNG (bản nâng cấp):** Tool sử dụng kỹ thuật **`execCommand('insertText')`** kết hợp quét vùng chọn DOM để gõ chữ trực tiếp vào Slate.js Rich Text Editor của Google Flow. Nhờ vậy:
> 1. **KHÔNG cần dùng `chrome.debugger`**: Không còn cảnh báo thanh vàng khó chịu trên Chrome.
> 2. **Hỗ trợ mở DevTools (F12)** song song thoải mái khi đang chạy để xem log gỡ lỗi.
> 3. Không yêu cầu các quyền đặc biệt phức tạp khi cài đặt.
>
> Quy trình: dán prompt (mỗi dòng 1 cái) → Bắt đầu → tool tự gõ + tự động Click/Enter gửi yêu cầu → chờ ảnh xong → tự tải về `Downloads/[thư mục]/` → prompt tiếp theo.
