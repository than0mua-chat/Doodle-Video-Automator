...
async function run() {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    // 1) Điền prompt và gửi trực tiếp bằng Content Script (không cần dùng debugger)
    const typed = await sendToTab(tab.id, {
      type: "SUBMIT_PROMPT",
    ...
    if (!typed || !typed.ok) {
      ...
      if (typed && typed.error) {
        setConn(false, typed.error);
      } else {
        setConn(false, "Không thể gõ hoặc gửi prompt.");
      }
      renderQueue();
      break;
    }
